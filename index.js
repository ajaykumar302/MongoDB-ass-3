...
async function main() {
  try {
    ...
    const data = await EmpModel.insertMany(employeeList);
    ...
    const salary = await EmpModel.find({
      salary: { $gt: 30000 },
    });
    console.log(salary);
    ...
    const overallExp = await EmpModel.find({
      overallExp: { $gte: 2 },
    });
    ...
    const yearGrad = await EmpModel.find({
      $and: [
        {
          yearGrad: { $gt: 2015 },
        },
    ...
    const salary1 = await EmpModel.updateMany(
      {
        salary: { $gt: 15000 },
      },
      {
    ...
    const company = await EmpModel.deleteMany({
      lastCompany: "Y",
    });
    console.log(company);
